...
### [x] C06 — Reference benchmark A: description-to-label benchmark
...
- Tasks:
  ...
  - [x] Add original description-to-label cases.
  - [x] Add smoke subset via the reserved `smoke` case tag (at least one `smoke`-tagged case).
  - [x] Add benchmark README with limitations and non-endorsement language.
  - [x] Add a checked-in non-stub prediction sample (`benchmarks/description-label/sample_predictions`) and run the `--predictions` non-stub scoring path; record a schema-valid run-record.
...
- Review-fix evidence (C06): C06 review findings addressed in `benchmarks/description-label/cases/snow-indy-grab.yaml` and `benchmarks/description-label/README.md`: (1) the `snow-indy-grab` case description now says the rear hand grabs the toe-side edge (correct for an indy grab) instead of the heel-side edge, so the description matches the `expected: "indy grab"` label; (2) the README smoke-subset composition now reads "one trivial, two easy, and one medium" to match the actual `smoke`-tagged cases (`skate-ollie` trivial, `skate-kickflip` and `snow-ollie` easy, `surf-cutback` medium). Per assignment, no gates/tests/formatters/git were run for this review-only fix.
...
- Review-fix evidence (C07): C07 review findings addressed in `src/ai_bench/sandbox.py`, `src/ai_bench/scoring.py`, `tests/fixtures/sandbox/git-benchmark/cases/*.yaml`, `tests/test_sandbox.py`, `tests/test_sandbox_hardening.py`, `tests/test_sandbox_integration.py`: (1) in-process and bwrap backends now validate the full git argv through a strict safe-subcommand/option allowlist (`_validate_git_argv`) BEFORE invoking host git, rejecting global `-c`/`--config-env` config injection, alias expansion, hooks/pagers/external helpers (`--exec`, `--hooks-path`, `--pager`, `--git-dir`, `--work-tree`, etc.), network-capable subcommands, and unvetted per-subcommand options; a regression test for `git -c alias.x='!sh -c ...'` shell escape is added; (2) bwrap backend no longer passes `--share-net` (network namespace stays unshared via `--unshare-all`), with a static-source assertion test; (3) `RepoStateVerifier` now enforces `contains` against the snapshot diff and fails closed when content is unavailable, and rejects `sha256` assertions as unsupported by the path-only snapshot instead of silently passing them unchecked; (4) `_check_symlink_escape` now walks ancestors under the sandbox root and rejects symlinked-directory ancestor escapes before any file/directory is created/written through them (regression tests for `file.write`/`file.mkdir` through a symlinked ancestor); (5) the checked-in sandbox fixture's smoke scripts now run `git init`/`git config`/initial commit inside the sandbox-safe script so the fixture is self-sufficient without embedded `.git` or out-of-band test init, and the integration test no longer calls `_git_init` out-of-band. Per assignment, no pytest/gates/formatters/git were run; verification used targeted Python smoke snippets that compiled the touched files and exercised all five review regressions (15 assertions) plus the existing `RepoStateVerifier` unit-test logic.
